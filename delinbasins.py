    ...
    def run(self):
        ...
        for i in range(int(blocks_num)):
            ...
            for j in range(int(blocks_num)):
                if blockcityin.getAttributes("BlockID"+str(j+1)).getAttribute("Status") == 0:
                    continue
                if blockcityin.getAttributes("BlockID"+str(j+1)).getAttribute("downstrID") == currentID:
                    upstreamIDs[i].append(j+1)
                if blockcityin.getAttributes("BlockID"+str(j+1)).getAttribute("downstrID") == -1:
                    if blockcityin.getAttributes("BlockID"+str(j+1)).getAttribute("drainto_ID") == currentID:
                        upstreamIDs[i].append(j+1)
