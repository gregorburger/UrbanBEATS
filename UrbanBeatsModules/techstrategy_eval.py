...
class techstrategy_eval(Module):
	...
	self.desAttr.getAttribute("scoringmatrix_default")
	self.desAttr.getAttribute("scoring_include_all")
	self.desAttr.getAttribute("scoringmatrix_path")
	self.desAttr.getAttribute("bottomlines_tech_n")
	self.desAttr.getAttribute("bottomlines_env_n")
	self.desAttr.getAttribute("bottomlines_ecn_n")
	self.desAttr.getAttribute("bottomlines_soc_n")
	self.desAttr.getAttribute("bottomlines_tech")
	self.desAttr.getAttribute("bottomlines_env")
	self.desAttr.getAttribute("bottomlines_ecn")
	self.desAttr.getAttribute("bottomlines_soc")
	self.desAttr.getAttribute("bottomlines_tech_w")
	self.desAttr.getAttribute("bottomlines_env_w")
	self.desAttr.getAttribute("bottomlines_ecn_w")
	self.desAttr.getAttribute("bottomlines_soc_w")
	self.desAttr.getAttribute("bottomlines_tech_p")
	self.desAttr.getAttribute("bottomlines_env_p")
	self.desAttr.getAttribute("bottomlines_ecn_p")
	self.desAttr.getAttribute("bottomlines_soc_p")
	self.desAttr.getAttribute("scope_stoch")
	self.desAttr.getAttribute("score_method")
	self.desAttr.getAttribute("tech2strat_method")
	self.desAttr.getAttribute("topranklimit")
	self.desAttr.getAttribute("conf_int")
	#self.desAttr.getAttribute("ingroup_sorting")
	self.desAttr.getAttribute("ingroup_scoring")
	self.desAttr.getAttribute("eval_mode")
	self.desAttr.getAttribute("ranktype")

	self.lotStrats = View("LotStrats",COMPONENT,READ)
	self.lotStrats.addAttribute("BlockID")
	self.lotStrats.addAttribute("TotalLotOptions")

    	self.streetStrats = View("StreetStrats", COMPONENT,READ)
	self.streetStrats.addAttribute("BlockID")
	self.streetStrats.addAttribute("TotalCombinations")
	
	self.streetStratsCombo = View("StreetStratsCombo",COMPONENT,READ)
	self.streetStratsCombo.addAttribute("Name")
	self.streetStratsCombo.addAttribute("TotalOptions")

	self.neighStrats = View("NeighStrats",COMPONENT,READ)
	self.neighStrats.addAttribute("BlockID")
	self.neighStrats.addAttribute("TotalCombinations")

	self.neighStratsCombo = View("NeighStratsCombo",COMPONENT,READ)
	self.neighStratsCombo.addAttribute("Name")
	self.neighStratsCombo.addAttribute("TotalOptions")

	self.precStrats = View("PrecStrats",COMPONENT,READ)
	self.precStrats.addAttribute("BlockID")
	self.precStrats.addAttribute("TotalCombinations")

	self.precStratsCombo = View("PrecStratsCombo",COMPONENT,READ)
    ...
    def getLotUUID(self, lotid,city):
	try:
		key = self.LotIDtoUUID[lotid]
	except KeyError:
		key = ""
	return city.getComponent(key)

    def getStreetUUID(self, streetid,city):
	try:
		key = self.StreetIDtoUUID[streetid]
	except KeyError:
		key = ""
	return city.getComponent(key)

    def getNeighUUID(self, neighid,city):
	try:
		key = self.NeighIDtoUUID[neighid]
	except KeyError:
		key = ""
	return city.getComponent(key)

    def getPrecUUID(self, precid,city):
	try:
		key = self.PrecIDtoUUID[precid]
	except KeyError:
		key = ""
	return city.getComponent(key)
    ...
    def initLotIDtoUUID(self, city):
	lotuuids = city.getUUIDsOfComponentsInView(self.lotStrats)
        for lotuuid in lotuuids:
            lot = city.getComponent(lotuuid)
            ID = int(round(lot.getAttribute("BlockID").getDouble()))
	    self.LotIDtoUUID[ID] = lotuuid

    def initStreetIDtoUUID(self, city):
	streetuuids = city.getUUIDsOfComponentsInView(self.streetStrats)
        for streetuuid in streetuuids:
            street = city.getComponent(streetuuid)
            ID = int(round(street.getAttribute("BlockID").getDouble()))
	    self.StreetIDtoUUID[ID] = streetuuid	

    def initNeighIDtoUUID(self, city):
	neighuuids = city.getUUIDsOfComponentsInView(self.neighStrats)
        for neighuuid in neighuuids:
            neigh = city.getComponent(neighuuid)
            ID = int(round(neigh.getAttribute("BlockID").getDouble()))
	    self.NeighIDtoUUID[ID] = neighuuid

    def initPrecIDtoUUID(self, city):
	precuuids = city.getUUIDsOfComponentsInView(self.precStrats)
	...
	self.initLotIDtoUUID(city)
	self.initStreetIDtoUUID(city)
	self.initNeighIDtoUUID(city)
	self.initPrecIDtoUUID(city)
        rand.seed()
	...
	strvec = city.getUUIDsOfComponentsInView(self.desAttr)
        ...
        basins = map_attr.getAttribute("TotalBasins").getDouble()       #total number of basins in map
        ...
        #---------------------------------------------------------------------------------#
        if des_attr.getAttribute("scoringmatrix_default").getString() == 1:
            score_type = 1      #uses UBeats matrix     (coming soon)
            mca_fname = "default"
            mca_allcrit = des_attr.getAttribute("scoring_include_all").getDouble() 
        else:
            score_type = 0      #uses custom matrix
            mca_fname = des_attr.getAttribute("scoringmatrix_path").getString()
            mca_techN = des_attr.getAttribute("bottomlines_tech_n").getDouble()                  #number of criteria involved (if not "default" score type)
            mca_envN = des_attr.getAttribute("bottomlines_env_n").getDouble() 
            mca_ecnN = des_attr.getAttribute("bottomlines_ecn_n").getDouble() 
            mca_socN = des_attr.getAttribute("bottomlines_soc_n").getDouble() 
            
        mca_tech = des_attr.getAttribute("bottomlines_tech").getDouble()                     #Booleans to include criteria
        mca_env = des_attr.getAttribute("bottomlines_env").getDouble() 
        mca_ecn = des_attr.getAttribute("bottomlines_ecn").getDouble() 
        mca_soc = des_attr.getAttribute("bottomlines_soc").getDouble() 
        
        mca_techW = des_attr.getAttribute("bottomlines_tech_w").getDouble()                  #weightings of different criteria
        mca_envW = des_attr.getAttribute("bottomlines_env_w").getDouble() 
        mca_ecnW = des_attr.getAttribute("bottomlines_ecn_w").getDouble() 
        mca_socW = des_attr.getAttribute("bottomlines_soc_w").getDouble() 
        
        mca_techP = des_attr.getAttribute("bottomlines_tech_p").getDouble()                 #pareto exploration mode increments
        mca_envP = des_attr.getAttribute("bottomlines_env_p").getDouble() 
        mca_ecnP = des_attr.getAttribute("bottomlines_ecn_p").getDouble() 
        mca_socP = des_attr.getAttribute("bottomlines_soc_p").getDouble() 
        
        mca_stoch = des_attr.getAttribute("scope_stoch").getDouble() 
        mca_method = des_attr.getAttribute("score_method").getString()
        mca_t2s = des_attr.getAttribute("tech2strat_method").getString()
        
        mca_topranklimit = des_attr.getAttribute("topranklimit").getDouble()                 #limit ranking to the top x where x is the topranklimit
        mca_confint = des_attr.getAttribute("conf_int").getDouble() /100                     #confidence interval converted to proportion
        #mca_ingroupscore = des_attr.getAttribute("ingroup_sorting").getString()
        ...
        mca_fname = des_attr.getAttribute("scoringmatrix_path").getString()
        eval_mode = des_attr.getAttribute("eval_mode").getString()
        ranktype = des_attr.getAttribute("ranktype").getString()                      #CI = confidence interval, RK = top ranking
		...
		continue
            ...
            totalimparea = currentAttList.getAttribute("ResTIArea").getDouble()
            
            #--STEP 1-- Get the attribute lists for different strategies -----------------#
            lot_strats = self.getLotUUID(currentID,city)
            street_strats = self.getStreetUUID(currentID,city)
            neigh_strats = self.getNeighUUID(currentID,city)
            

            lot_options = lot_strats.getAttribute("TotalLotOptions").getDouble()
            street_totcombos = street_strats.getAttribute("TotalCombinations").getDouble()
            neigh_totcombos = neigh_strats.getAttribute("TotalCombinations").getDouble()
            ...
            for a in range(int(lot_options)):                                                           #each element in tech_array contains one
                currentoption = lot_strats.getAttribute("LotOption_"+str(a+1)).getString()                    #WSUD Object as written in technology.py
            ...
            for a in range(len(lot_alts)):
                ...
                for b in range(len(street_alts)):     #begin looping from 0.25, not from zero
                    ...
                    if street_deg == 0:
                        ...
                        continue
		    name = str(currentID)+"_Street_"+str(lot_deg)+"_"+str(street_deg)
                    street_strats_combo = self.getComboComponentOfName(city,self.streetStratsCombo,name)#street_opps.getAttributes("BlockID"+str(currentID)+"_Street_"+str(lot_deg)+"_"+str(street_deg))
		    if street_strats_combo == "":
			continue
		    street_options = street_strats_combo.getAttribute("TotalOptions").getDouble()
                    for c in range(int(street_options)):
                        currentoption = street_strats_combo.getAttribute("StreetOption_"+str(c+1)).getString()
            ...
            for a in range(len(lot_alts)):
                ...
                for b in range(len(neigh_alts)):
                    ...
                    if neigh_deg == 0:
                        ...
                        continue
		    name = str(currentID)+"_Neigh_"+str(lot_deg)+"_"+str(neigh_deg)
		    neigh_strats_combo = self.getComboComponentOfName(city,self.neighStratsCombo,name)#neigh_opps.getAttributes("BlockID"+str(currentID)+"_Neigh_"+str(lot_deg)+"_"+str(neigh_deg))
                    if neigh_strats_combo == "":
			continue
		    neigh_options = neigh_strats_combo.getAttribute("TotalOptions").getDouble()
                    for c in range(int(neigh_options)):
                        currentoption = neigh_strats_combo.getAttribute("NeighOption_"+str(c+1)).getString()
                        currentoption = currentoption.split(',')
                        currentoption.remove('')
...
#                print block_strategies_matrix[i][j]
		...
		print "BTIA : " + str(btia)
            ...
            for currentBlockID in basinblockIDs:
                ...
                currentblock_opps = self.getPrecUUID(currentBlockID,city)#prec_opps.getAttributes("BlockID"+str(currentBlockID)+"_Prec")
		    ...
		    name = str(currentBlockID)+"_Prec_"+str(prec_deg)
                    prec_strats_combo = self.getComboComponentOfName(city,self.precStratsCombo,name)#prec_opps.getAttributes("BlockID"+str(currentBlockID)+"_Prec_"+str(prec_deg))
                    if prec_strats_combo == "":
			continue
		    if prec_strats_combo.getAttribute("TotalOptions").getDouble() == 0:
                        print "No precinct options available for BlockID"+str(currentBlockID)+" at prec deg: "+str(prec_deg)
                        continue
                    else:
                        total_options += prec_strats_combo.getAttribute("TotalOptions").getDouble()
		...
		#katze
                ...
                downstreamString = currentAttList.getAttribute("BasinDownBlocks").getString()
		    ...
		    name = "BlockID"+str(currentBlockID)+"_Prec_"+str(prec_deg)
                    prec_strats_combo = self.getComboComponentOfName(city,self.precStratsCombo,name)#prec_opps.getAttributes("BlockID"+str(currentBlockID)+"_Prec_"+str(prec_deg))
                    if prec_strats_combo == "":
			continue
		    total_options = prec_strats_combo.getAttribute("TotalOptions")
                    ...
                    for opts in range(int(total_options)):
                        ...
                        optionstring = prec_strats_combo.getAttribute("PrecOption_"+str(opts+1)).getString()
...
#            print region_bin_scores
            ...
            for j in range(len(basinblockIDs)):
                currentblockID = basinblockIDs[j]
		cumu_Aimp_treated += self.getBlockUUID(currentblockID,city).getAttribute("IAServiced").getDouble()
                cumu_Aimp_treated += self.getBlockUUID(currentblockID,city).getAttribute("UpstrImpTreat").getDouble()
                tot_Aimp_in_basin += self.getBlockUUID(currentblockID,city).getAttribute("ResTIArea").getDouble()
        ...
        #########################################################################################################################
            ...
            for iterations in range(20):       #10 options, can be changed                                    <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< SET THE ITERATIONS FOR STRATEGY CONSTRUCTION <<<<<<<<<<<<<<<<<<
                ...
                for pID in range(len(prec_blocks_partakeIDs)):
                    ...
                    completeAimp_subbasin = self.getBlockUUID(currentblockID,city).getAttribute("ResTIArea").getDouble()
                    totalblockAimp_subbasin = self.getBlockUUID(currentblockID,city).getAttribute("IAServiced").getDouble()
                    totalprectreat_subbasin = self.getBlockUUID(currentblockID,city).getAttribute("UpstrImpTreat").getDouble()

		    for sbID in upstreamIDs:
                        completeAimp_subbasin += self.getBlockUUID(sbID,city).getAttribute("ResTIArea").getDouble()
                        totalblockAimp_subbasin += self.getBlockUUID(sbID,city).getAttribute("IAServiced").getDouble()
                        totalprectreat_subbasin += self.getBlockUUID(sbID,city).getAttribute("UpstrImpTreat").getDouble()
		    ...
		    #### Add this information to the basin_strategy Object ####
                    ...
                    for rbID in remain_upstreamIDs:             #loop across the remaining IDs If there are none, loop won't start
                        ...
                        print "Block ID: ", rbID
                        #get impervious area
                        block_Aimp = self.getBlockUUID(rbID,city).getAttribute("ResTIArea").getDouble()
...
#                    continue
            ...
            for i in range(len(basinblockIDs)):
                ...
                for j in range(len(blockscale_names)):
                    ...
                    wsud_attr = Component()
		    city.addComponent(wsud_attr, self.wsudAttr)
                    wsud_attr.addAttribute("ScaleN", j) #0 = LOT, 1 = STREET, 2 = NEIGHBOURHOOD, 3 = PRECINCT
                    wsud_attr.addAttribute("Scale", scale)
                    wsud_attr.addAttribute("Location", currentBlockID)
                    wsud_attr.addAttribute("Type", current_wsud.getType())
                    wsud_attr.addAttribute("TypeN", system_type_numeric[system_type_matrix.index(current_wsud.getType())])
                    wsud_attr.addAttribute("Qty", 1)
                    wsud_attr.addAttribute("GoalQty", 1) #set as 1 for lot now and update in the implement algorithm
                    wsud_attr.addAttribute("Degree", inblock_degs[j])
                    wsud_attr.addAttribute("SysArea", current_wsud.getSize())
                    wsud_attr.addAttribute("Status", 0) #0 = Planned, 1 = constructed
                    wsud_attr.addAttribute("Year", 9999) #Year constructed
                    wsud_attr.addAttribute("EAFact", current_wsud.getAreaFactor())
                    wsud_attr.addAttribute("ImpT", current_wsud.getAreaServed())
                    wsud_attr.addAttribute("CurImpT", 0) #current impervious area treated
                ...
                if outblock_strat == None:
                    ...
                    wsud_attr = Component()
		    city.addComponent(wsud_attr, self.wsudAttr)
                    wsud_attr.addAttribute("ScaleN", 3) #0 = LOT, 1 = STREET, 2 = NEIGHBOURHOOD, 3 = PRECINCT
                    wsud_attr.addAttribute("Scale", "P")
                    wsud_attr.addAttribute("Location", currentBlockID)
                    wsud_attr.addAttribute("Type", outblock_strat.getType())
                    wsud_attr.addAttribute("TypeN", system_type_numeric[system_type_matrix.index(outblock_strat.getType())])
                    wsud_attr.addAttribute("Qty", 1)
                    wsud_attr.addAttribute("GoalQty", 1) #set as 1 for lot now and update in the implement algorithm
                    wsud_attr.addAttribute("Degree", outblock_strat_deg)
                    wsud_attr.addAttribute("SysArea", outblock_strat.getSize())
                    wsud_attr.addAttribute("Status", 0) #0 = Planned, 1 = constructed
                    wsud_attr.addAttribute("Year", 9999) #Year constructed
                    wsud_attr.addAttribute("EAFact", outblock_strat.getAreaFactor())
                    wsud_attr.addAttribute("ImpT", outblock_strat.getAreaServed())
                    wsud_attr.addAttribute("CurImpT", 0) #current impervious area treated
                    wsud_attr.addAttribute("Upgrades", 0)
                ...
                for lpsc in loopscales:
                    ...
                    if wsud_attr.getAttribute("Location") > 0:
                        print wsud_attr.getAttribute("Type").getString()
                        print wsud_attr.getAttribute("Scale").getString()
                        print "Transfer the system"

                        coordinates = offsets_matrix[loopscales.index(lpsc)]

                        wsud_attr.addAttribute("ScaleN", loopscales.index(lpsc))
                        '''
